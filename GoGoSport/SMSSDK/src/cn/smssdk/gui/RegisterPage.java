...
//验证成功直接跳转到填写密码界面
	...
	private void checkPhoneNum(final String phone,  String code) {
		...
		//获取手机号
		phone2=etPhoneNum.getText().toString();
		Properties properties=MyPropertiesUtil.getProperties(getContext());
		String u=properties.getProperty("u");
		// 发送请求，判断用户是否存在
		String url =u+"AndroidTelYanZheng?phone="
				+ phone2;
		// 1.初始化RequestQueue对象
		...
		StringRequest stringRequsset = new StringRequest(
				url, new Listener<String>() {
					...
					public void onResponse(String response) {
						...
						if (response.equals("true")) {// 说明已经注册过，提示用户去登录
							Toast.makeText(
									getContext(),
									"您已经注册过了，可以直接登录哟", 1000)
									.show();
						} else {// 说明是第一次注册，将tel存入缓存，跳转到发送消息对话框
							String str="86";
							showDialog(phone, str);						
							//存入缓存
							SharedPreferences mySharedPreferences = getContext()
