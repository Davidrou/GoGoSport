					...
					public void run() {
						if (listView.getFirstVisiblePosition() == 0
							...
							Toast.makeText(ChatActivity.this,
									"no_more_messages", Toast.LENGTH_SHORT)
									.show();
						}
						swipeRefreshLayout.setRefreshing(false);
					}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == REQUEST_CODE_CONTEXT_MENU) {
			...
			case RESULT_CODE_FORWARD: // 转发消息
				Toast.makeText(this, "转发消息ACtivity", Toast.LENGTH_SHORT).show();
...
//		}
	...
	public void toGroupDetails(View view) {
		if (group == null) {
			Toast.makeText(getApplicationContext(), "群组未找到", 0).show();
			return;
		}
		if (chatType == CHATTYPE_GROUP) {
			Toast.makeText(ChatActivity.this, "进入群组详情AC", Toast.LENGTH_SHORT)
					.show();
			// startActivityForResult(
	...
	class PressToSpeakListen implements View.OnTouchListener {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			case MotionEvent.ACTION_MOVE: {
				if (event.getY() < 0) {
					recordingHint.setText("放开手指  取消发送");
					recordingHint
